...
import { STORAGE_PREFIX, DEFAULT_SHEETS, calculateRemainingTime, calculateAccountReminder } from '../utils/dataRepair';
...
export default function DashboardAlerts({ onNavigateSheet, mode = 'dashboard' }) {
    ...
    const loadAllData = () => {
        ...
        DEFAULT_SHEETS.forEach(sheet => {
            try {
                const raw = localStorage.getItem(`${STORAGE_PREFIX}${sheet.id}`);
                if (raw) {
                    const parsed = JSON.parse(raw);
                    recordsMap[sheet.id] = Array.isArray(parsed) ? parsed : [];
                } else {
                    recordsMap[sheet.id] = [];
                }
            } catch {
        ...
        setAllRecordsBySheet(recordsMap);
    };
